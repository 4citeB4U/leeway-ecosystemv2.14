...
function state() {
  ...
  return {
    schema: "leeway-capability-center-runtime-state-v23",
    centerId,
    centerType,
    status: "WARM_READY",
    startedAt,
...
<h1>${centerType}</h1>
<div class="card">
  <p><b>Center ID:</b> ${centerId}</p>
  <p><b>Status:</b> <span class="badge">${s.status}</span></p>
  <p><b>Item Count:</b> ${s.itemCount}</p>
  <p><b>Dispatch Count:</b> ${s.dispatchCount}</p>
</div>
<table>
<thead>
<tr>
...
const server = http.createServer(async (req, res) => {
  ...
  if (path === "/") return sendHtml(res, 200, html());
...
server.listen(port, "0.0.0.0", () => {
  console.log(JSON.stringify({
    event: "LEEWAY_CAPABILITY_CENTER_STARTED",
    centerId,
    centerType,
    port
  }));
