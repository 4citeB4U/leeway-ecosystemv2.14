...
#
# ID: LEEWAY_APP::AGENT_LEE::VALIDATION::VALIDATE_AGENT_LEE_VOICE_EARS
# CLASSIFICATION: VALIDATION
# OWNER: LeeWay Local Runtime Stabilization
# PURPOSE: Diagnostic-first proof that Agent Lee voice ears can status-check and transcribe a known WAV sample.
...
$ReceiptPath = Join-Path $ReceiptDir "agent-lee-voice-ears-proof-$Stamp.json"
...
function Invoke-AgentLeeProbe {
  ...
  $probe.transcriptEnglish = [string](Get-NestedFirstValue -InputObject $probe.parsed -Paths @(
    "transcript.english",
    "result.transcript.english"
  ))
  $probe.captureMode = [string](Get-NestedFirstValue -InputObject $probe.parsed -Paths @(
    "transcript.captureMode",
    "audioInput.captureMode",
    "result.transcript.captureMode",
    "result.audioInput.captureMode"
  ))
  $probe.routerReady = Get-NestedFirstValue -InputObject $probe.parsed -Paths @("routerReady", "result.routerReady")
  $probe.localWhisperAvailable = Get-NestedFirstValue -InputObject $probe.parsed -Paths @(
    "providers.local_whisper.available",
    "result.providers.local_whisper.available"
  )
...
function Write-CaseSummary {
  ...
  Write-Host "routerReady: $($Case.routerReady)"
  Write-Host "localWhisperAvailable: $($Case.localWhisperAvailable)"
  Write-Host "captureMode: $($Case.captureMode)"
...
function Test-StatusCase {
  ...
  if ($Case.routerReady -ne $true) { $failures.Add("router_not_ready") }
  return ,$failures
}

function Test-ListenCase {
  param([Parameter(Mandatory = $true)]$Case)

  $failures = New-Object System.Collections.Generic.List[string]
  if (-not $Case.ok -or $Case.statusCode -ne 200) { $failures.Add("http_not_200") }
  if ([string]$Case.status -notmatch 'PASS') { $failures.Add("status_not_pass") }
  if ($Case.captureMode -ne "transcribe") { $failures.Add("capture_mode_not_transcribe") }
  if ([string]::IsNullOrWhiteSpace([string]$Case.transcriptText)) { $failures.Add("transcript_empty") }
  if ($Case.transcriptText -match 'request timed out|downstream model backend did not respond|Backend note') { $failures.Add("stale_timeout_text_present") }
  return ,$failures
}

$Report = [ordered]@{
  schema = "leeway.agent-lee.voice-ears-proof.v1"
...
try {
  ...
  $listenProbe = Invoke-AgentLeeProbe -Name "voice_listen_sample" -Method "POST" -Url $VoiceListenUrl -JsonBody $listenBody -TimeoutSec 180 -VerboseRaw:$VerboseRaw
  ...
  $caseRows = @(
    [ordered]@{
      ...
      routerReady = $statusProbe.routerReady
      localWhisperAvailable = $statusProbe.localWhisperAvailable
      captureMode = $statusProbe.captureMode
    ...
    [ordered]@{
      name = "voice_listen_sample"
      url = $VoiceListenUrl
      ok = $false
      statusCode = $listenProbe.statusCode
      ms = $listenProbe.ms
      status = $listenProbe.status
      routerReady = $listenProbe.routerReady
      localWhisperAvailable = $listenProbe.localWhisperAvailable
      captureMode = $listenProbe.captureMode
      transcriptText = $listenProbe.transcriptText
      responseMode = $listenProbe.responseMode
      selectedBackend = $listenProbe.selectedBackend
      fallbackUsed = $listenProbe.fallbackUsed
      timeout = $listenProbe.timeout
      rawBody = $listenProbe.rawBody
      error = $listenProbe.error
      failures = @()
    }
  )

  $caseRows[0].failures = Test-StatusCase -Case $statusProbe
  $caseRows[1].failures = Test-ListenCase -Case $listenProbe
