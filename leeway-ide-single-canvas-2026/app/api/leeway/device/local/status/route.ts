...
async function proxyRuntimeFabric(req: NextRequest, fabricPath: string) {
  ...
  const resolvedPath = fabricPath;
  
  try {
    const method = req.method.toUpperCase();
    const response = await fetch(`${runtimeFabricUrl}${resolvedPath}`, {
      method,
      headers: {
    ...
    if (!response.ok && response.status === 404) {
      return NextResponse.json({
        ...
        details: `Runtime Fabric did not expose ${resolvedPath}.`,
...
export async function GET(req: NextRequest) {
  return proxyRuntimeFabric(req, '/device/local/status');
}