...
export async function loadOmniTerminalSnapshot(): Promise<OmniTerminalSnapshot> {
  ...
  try {
    ...
    const terminalPayload = (terminalProbe?.ok ? terminalProbe.payload : null) as Record<string, any> | null;
    ...
    const lastReceipt = normalizeReceiptSummary(terminalPayload?.lastReceipt)
      ?? normalizeReceiptSummary((receiptsProbe?.payload as Record<string, any>)?.lastReceipt)
