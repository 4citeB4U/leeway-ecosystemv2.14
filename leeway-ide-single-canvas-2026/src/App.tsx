...
export default function App() {
  ...
  const executeApprovedCommandPlan = useCallback(async (plan?: Record<string, any>, sourceNodeId?: string) => {
    ...
    const stdout = String((result.payload as any)?.stdout || (result.receipt as any)?.stdout || "").trim();
    const stderr = String((result.payload as any)?.stderr || (result.receipt as any)?.stderr || "").trim();
    const exitCode = typeof (result.payload as any)?.exitCode === "number"
      ? (result.payload as any).exitCode
      : (result.receipt as any)?.exitCode;
    const receiptPath = String((result.payload as any)?.receiptPath || (result.receipt as any)?.receiptPath || "").trim();
