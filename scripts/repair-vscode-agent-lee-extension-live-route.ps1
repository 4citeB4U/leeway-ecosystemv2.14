...
$BackupDir = Join-Path $Root "Archive\backups\vscode-agent-lee-extension-live-route-$Stamp"
$LogDir = Join-Path $Root "Archive\logs\vscode-agent-lee-extension-live-route"
...
$ReceiptPath = Join-Path $ReceiptDir "agent-lee-vscode-extension-live-route-repair-$Stamp.json"
...
function Invoke-AdapterHello {
    $Body = @{
        ...
        max_tokens = 80
        provenance = @{
            origin = "powershell-extension-live-route-repair"
            workspaceRoot = $Root
...
function Patch-ExtensionLiveRoute {
    ...
    $Result.directRouterBefore = ($Text.Contains("127.0.0.1:8080/v1/chat/completions") -or $Text.Contains("localhost:8080/v1/chat/completions"))
    ...
    # Common env/config values.
    $Text = [regex]::Replace($Text, "(?i)(ADAPTER_URL\s*=\s*['""])[^'""]+(['""])", '${1}http://127.0.0.1:8787${2}')
    $Text = [regex]::Replace($Text, "(?i)(adapterUrl\s*=\s*['""])[^'""]+(['""])", '${1}http://127.0.0.1:8787${2}')
    $Text = [regex]::Replace($Text, "(?i)(baseUrl\s*=\s*['""]http://127\.0\.0\.1:)8080(['""])", '${1}8787${2}')
    $Text = [regex]::Replace($Text, "(?i)(baseUrl\s*=\s*['""]http://localhost:)8080(['""])", '${1}8787${2}')

    # Raise timeout constants to let Qwen3 cold-start.
    ...
    # Add a visible marker comment/config constant if no 8787 string exists.
    if (-not $Text.Contains("127.0.0.1:8787")) {
        $Marker = @"
/*
 * Agent Lee live route marker.
 * Official VS Code live path must use adapter first:
 * http://127.0.0.1:8787
 */
...
"@
    ...
    $Result.directRouterAfter = ($AfterText.Contains("127.0.0.1:8080/v1/chat/completions") -or $AfterText.Contains("localhost:8080/v1/chat/completions"))
...
Write-Host ""
Write-Host "=== Repair VS Code Agent Lee Extension Live Route ===" -ForegroundColor Cyan
...
$Receipt = [ordered]@{
    ...
    conclusion = $(if ($AdapterHello.ok -and $ExtensionPatch.usesAdapter8787After -and -not $ExtensionPatch.directRouterAfter) {
        "Backend is healthy and extension is now visibly adapter-first. Restart VS Code extension host and reload window."
