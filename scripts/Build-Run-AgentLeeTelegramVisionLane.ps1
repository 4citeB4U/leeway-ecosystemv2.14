...
ENV AGENT_LEE_APP_NAME=agent-lee-telegram-vision-lane
ENV AGENT_LEE_ARTIFACT_ROOT=/artifacts
ENV OLLAMA_BASE_URL=http://leeway_ollama:11434
ENV AGENT_LEE_VISION_MODEL=qwen2.5vl:7b
...
# Pull Telegram credentials from existing shell if present.
$EnvLines = @()
try {
...
$Receipt = @{
  verdict = "AGENT_LEE_TELEGRAM_VISION_LANE_BUILT"
  purpose = "phone photo through Telegram to local Qwen2.5-VL vision reply"
  container = $ContainerName
  image = $ImageName
  host_port = 8104
  status = $Status
  note = "If Telegram token is not present, add TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID."
  created_at = (Get-Date).ToString("o")
}

$ReceiptPath = Join-Path $Proof "AGENT_LEE_TELEGRAM_VISION_LANE_BUILD_$Stamp.receipt.json"
$Receipt | ConvertTo-Json -Depth 80 | Set-Content -Path $ReceiptPath -Encoding UTF8

notepad $ReceiptPath

Write-Host ""
Write-Host "Telegram vision lane running." -ForegroundColor Green
Write-Host "Status: http://127.0.0.1:8104/status" -ForegroundColor Cyan
Write-Host "Webhook endpoint: http://127.0.0.1:8104/telegram/webhook" -ForegroundColor Cyan
Write-Host "Local test endpoint: http://127.0.0.1:8104/test/describe-local" -ForegroundColor Cyan