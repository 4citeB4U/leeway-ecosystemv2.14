...
if ($Text -notmatch 'def get_telegram_offset') {
$Append = @'
...
$Receipt = @{
  verdict = "AGENT_LEE_TELEGRAM_VISION_SEND_AND_POLL_PATCHED"
  adds = @(
    "/test/send-telegram",
    "/test/describe-local-and-send",
    "/telegram/poll-once",
    "explicit Docker DNS 8.8.8.8 and 1.1.1.1"
  )
  status = $Status
  created_at = (Get-Date).ToString("o")
}

$ReceiptPath = Join-Path $Proof "AGENT_LEE_TELEGRAM_VISION_SEND_AND_POLL_PATCH_$Stamp.receipt.json"
$Receipt | ConvertTo-Json -Depth 80 | Set-Content -Path $ReceiptPath -Encoding UTF8

notepad $ReceiptPath

Write-Host ""
Write-Host "Telegram send/poll patch applied." -ForegroundColor Green
