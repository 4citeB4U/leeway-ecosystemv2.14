...
ENV AGENT_LEE_APP_NAME=agent-lee-character-cutout-lane
...
$Status = Invoke-RestMethod -Method Get -Uri "http://127.0.0.1:8102/status" -TimeoutSec 30

$Receipt = @{
  verdict = "AGENT_LEE_CHARACTER_CUTOUT_LANE_BUILT"
  purpose = "extract actual character only before true image-to-3d mesh"
  not_background = $true
  not_relief_card = $true
...
$ReceiptPath = Join-Path $Proof "AGENT_LEE_CHARACTER_CUTOUT_LANE_BUILD_$Stamp.receipt.json"
$Receipt | ConvertTo-Json -Depth 80 | Set-Content -Path $ReceiptPath -Encoding UTF8

notepad $ReceiptPath

Write-Host ""
Write-Host "Character cutout lane running." -ForegroundColor Green
Write-Host "Status: http://127.0.0.1:8102/status" -ForegroundColor Cyan
Write-Host "Cutout endpoint: http://127.0.0.1:8102/cutout/from-path" -ForegroundColor Cyan