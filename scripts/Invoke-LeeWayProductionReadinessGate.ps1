...
# Absolute proof law overlay. A weighted score cannot override lane-level proof gaps.
$proofLedgerPath = Join-Path $ReportsRoot "leeway-proof-ledger.json"
$proofLedgerStatus = "MISSING"
$proofLedgerSummary = $null
$falseReadyClaims = @()
$proofGaps = @()
$weightedGateWarning = $null
if (Test-Path -LiteralPath $proofLedgerPath) {
    try {
        $proofLedger = Get-Content -Raw -LiteralPath $proofLedgerPath | ConvertFrom-Json -ErrorAction Stop
        $proofLedgerStatus = "LOADED"
        $proofLedgerSummary = $proofLedger.summary
        $falseReadyClaims = @($proofLedger.falseReadyClaims)
        $proofGaps = @($proofLedger.lanes | Where-Object { $_.correctedStatus -ne "READY_PROVEN" })
        if ($proofGaps.Count -gt 0 -or $falseReadyClaims.Count -gt 0) {
            $weightedGateWarning = "WEIGHTED_PASS_WITH_BLOCKERS: proof ledger has $($proofGaps.Count) unproven lane(s) and $($falseReadyClaims.Count) overstated claim(s)."
            if ($score -ge 99 -and $criticalFails -eq 0 -and $criticalWarnings -eq 0) {
                $verdict = "LEEWAY_WEIGHTED_PASS_WITH_BLOCKERS"
                $productionAllowed = $false
            } elseif ($verdict -notmatch "BLOCKED") {
                $verdict = "LEEWAY_PARTIAL_PROOF_BLOCKERS_REMAIN"
                $productionAllowed = $false
            }
        }
    } catch {
        $proofLedgerStatus = "INVALID: $($_.Exception.Message)"
        $weightedGateWarning = "PROOF_LEDGER_INVALID_OR_UNREADABLE"
        $verdict = "LEEWAY_FALSE_COMPLETION_RISK_DETECTED"
        $productionAllowed = $false
    }
} else {
    $weightedGateWarning = "PROOF_LEDGER_MISSING"
    if ($score -ge 99 -and $criticalFails -eq 0 -and $criticalWarnings -eq 0) {
        $verdict = "LEEWAY_FALSE_COMPLETION_RISK_DETECTED"
        $productionAllowed = $false
    }
...
if ($weightedGateWarning) {
    Write-Host " PROOF WARNING:   $weightedGateWarning" -ForegroundColor Yellow
}
Write-Host ""
...
$report = [ordered]@{
    ...
    absoluteProofLawLoaded = (Test-Path -LiteralPath (Join-Path $WorkspaceRoot "LeeWay-Standards\standards\BOOK-80-ABSOLUTE-PROOF-AND-NO-FALSE-COMPLETION-LAW.md"))
    proofLedgerStatus = $proofLedgerStatus
    proofLedgerPath = $proofLedgerPath
    proofLedgerSummary = $proofLedgerSummary
    falseReadyClaims = $falseReadyClaims
    proofGaps = $proofGaps
    weightedGateWarning = $weightedGateWarning
    blockers          = @(
        ...
        if ($weightedGateWarning) { $weightedGateWarning }
    )
}
