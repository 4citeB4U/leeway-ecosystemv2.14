...
# Install TripoSR dependencies after CUDA/PyTorch are present.
RUN if [ -f requirements.txt ]; then python -m pip install --no-cache-dir -r requirements.txt; fi

WORKDIR /srv
COPY app /srv/app

ENV AGENT_LEE_APP_NAME=agent-lee-true-character-mesh-lane
...
Write-Host ""
Write-Host "Building Agent Lee true character mesh lane. This can take a while..." -ForegroundColor Cyan
...
$State = docker inspect $ContainerName --format "Status={{.State.Status}} ExitCode={{.State.ExitCode}} OOMKilled={{.State.OOMKilled}} Error={{.State.Error}}"
$Status = Invoke-RestMethod -Method Get -Uri "http://127.0.0.1:8103/status" -TimeoutSec 60

$Receipt = @{
  verdict = "AGENT_LEE_TRUE_CHARACTER_MESH_LANE_BUILT"
  backend = "TripoSR"
  base = "nvidia/cuda:12.4.1-devel-ubuntu22.04"
  purpose = "real statue-like 3D mesh, not shell/card/relief"
...
$ReceiptPath = Join-Path $Proof "AGENT_LEE_TRUE_CHARACTER_MESH_LANE_BUILD_$Stamp.receipt.json"
$Receipt | ConvertTo-Json -Depth 100 | Set-Content -Path $ReceiptPath -Encoding UTF8

notepad $ReceiptPath

Write-Host ""
Write-Host "True character mesh lane running." -ForegroundColor Green
