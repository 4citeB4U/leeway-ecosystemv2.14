...
def try_skill_router_first(text):
    ...
    answer = data.get("answer") or "I routed that through the Agent Lee skill router."
    receipt = ((data.get("receipt") or {}).get("receipt_path") or "")
    ...
    if isinstance(result_data, dict):
        document = result_data.get("document")
        if isinstance(document, dict):
            extra = "\n\nDocument: " + str(document.get("path", ""))

    receipt_text = ""
    if receipt:
        receipt_text = "\n\nSkill receipt: " + str(receipt)
