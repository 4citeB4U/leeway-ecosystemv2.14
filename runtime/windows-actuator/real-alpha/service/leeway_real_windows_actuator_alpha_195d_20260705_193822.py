...
APP_VERSION = "LEEWAY_EXECUTE_APPROVED_OPEN_APP_BOM_SAFE_ISOLATED_MODULE_2026-07-05"
MODE = "ONE_ACTION_APPROVED_OPEN_APP_NOTEPAD_195D"
...
app = FastAPI(title="Leeway Real Windows Actuator Alpha 195D", version=APP_VERSION)
...
def blocked(action: str):
    return {
        ...
        "allowed": False,
        "executed": False,
        "reason": "one_action_195d_blocks_" + action,
        "mode": MODE,
    }
...
def real_open_app(req: RealRequest):
    ...
    executed = {
        ...
        "used": True,
        "source": "script_195d_bom_safe_isolated_module",
    }

    executed_path = EXECUTED_DIR / f"{req.approval_id}.executed-real-open-app-195d.approval.json"
    receipt_path = RECEIPT_DIR / f"real-open-app-195d-{req.approval_id}-{datetime.now().strftime('%Y%m%d-%H%M%S')}.receipt.json"
    ...
    return {
        ...
        "allowed": True,
        "executed": True,
        "reason": "approved_one_action_real_open_app_executed_195d",
        "mode": MODE,
        "approval_id": req.approval_id,
        "process_pid": proc.pid,
