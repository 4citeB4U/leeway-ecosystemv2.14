# LEEWAY-SEAFILE-DATABASE-CUTOVER-V1
...
[IO.File]::WriteAllBytes((Join-Path $backup 'production.configuration.dpapi.bin'),$encrypted)
...
New-Item -ItemType Directory (Join-Path $backup 'original-app-data') -Force|Out-Null
& docker cp 'leeway-seafile:/shared/.' (Join-Path $backup 'original-app-data')
if($LASTEXITCODE){& docker start $app.Id|Out-Null;throw 'Application data backup failed'}
$renamed=$false;$disconnected=$false;$newId=$null;$success=$false
try{
 ...
 & docker start $app.Id|Out-Null;if($LASTEXITCODE){throw 'Production app restart failed'}
 $ready=$false
 ...
 @{status='CUTOVER_FAILED_ROLLBACK_ATTEMPTED';failure=$failure;time=(Get-Date).ToUniversalTime().ToString('o')}|ConvertTo-Json|Set-Content (Join-Path $backup 'cutover-failure.json') -Encoding UTF8
 throw $failure
}
