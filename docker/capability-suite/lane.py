def create_app(settings):
    ...
    RUNTIME_NAME = settings.get("LEEWAY_RUNTIME_NAME", "leeway_runtime")
    RUNTIME_TITLE = settings.get("LEEWAY_RUNTIME_TITLE", "Leeway Runtime")
    RUNTIME_LANE = settings.get("LEEWAY_RUNTIME_LANE", "runtime")
    RUNTIME_PORT = int(settings.get("LEEWAY_RUNTIME_PORT", "5330"))
    
    STATE_DIR = Path(settings.get("LEEWAY_RUNTIME_STATE", "/state"))
    WORK_DIR = STATE_DIR / "work-orders"
    RECEIPT_DIR = Path(settings.get("LEEWAY_RECEIPT_DIR", "/app/receipts"))
    ...
    try:
        TRUTH = json.loads(settings.get("LEEWAY_RUNTIME_TRUTH_JSON", "{}"))
    except Exception:
        TRUTH = {}
    
    try:
        CAPABILITIES = json.loads(settings.get("LEEWAY_RUNTIME_CAPABILITIES_JSON", "[]"))
    except Exception:
        CAPABILITIES = []
    ...
    def health():
        try:
            STATE_DIR.stat(); RECEIPT_DIR.stat()
            if not os.access(STATE_DIR, os.W_OK) or not os.access(RECEIPT_DIR, os.W_OK):
                raise OSError('Storage is not writable')
        except OSError:
            return JSONResponse(status_code=503, content={'ok': False, 'runtime': RUNTIME_NAME, 'reason': 'STORAGE_UNAVAILABLE'})
        return {
            "ok": True,
            "runtime": RUNTIME_NAME,
            "title": RUNTIME_TITLE,
    ...
    try:
        _leeway_service_name = RUNTIME_NAME
